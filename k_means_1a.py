...
def distEuclidean(u,v):
    temp=0
    m=len(u)
    row1=u#constrainedR[u]
    row2=v#constrainedR[v]
    try:
        for col in range(m):
            if row1[col]>0:
                if row2[col]>0:
                    temp=temp+pow(row1[col]-row2[col],2)
    except:
...
#y=stats()
k=2 
centroids, cluster_assignments = kmeans_custom_distance(constrainedR, k, distEuclidean)
